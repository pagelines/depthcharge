...
var Block = function (container) {
  ...
  if(!(/Android|iPhone|iPad|iPod|BlackBerry|Windows Phone/i).test(navigator.userAgent || navigator.vendor || window.opera) || this.config.mobile ){
    dc_charter = true;
  }
...
function prime_dc_interface(){
  ...
  jQuery('.checkgroup-sprites > .checkbox').each(function(){
    ...
    if ( target.css('display') != 'none' ){
      outcome = jQuery(this).find('.checkbox-toggle').val();
      if ( outcome == 1 ){
        outcome = true;
      } else {
        outcome = false;
  ...
  jQuery('.checkgroup-sprites > .checkbox').change(function(){
    var target = jQuery(this);
    var outcome = jQuery(this).find('.checkbox-toggle').val();
    if ( outcome == 1 ){
      outcome = true;
    } else {
      outcome = false;
